...
db.execute("CREATE TABLE IF NOT E")
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not preshare:
            ...
            db.execute("UPDATE stock SET shares=? WHERE symbol=? AND user_id=?", num+preshare[0]['shares'] ,stock["symbol"], session["user_id"])

        availmon -= num * stock_cost
        db.execute("UPDATE users SET cash = ? WHERE id = ?", availmon, session["user_id"])

        db.execute("INSERT INTO history(user_id, symbol, shares, price, transacted) VALUES(?, ?, ?, ?, ?)", session['user_id'], stock['symbol'], num, stock_cost, dt_string )
        return redirect("/")
    else:
...
def sell():
    ...
    if request.method == "POST":
        ...
        preshare = db.execute("SELECT shares FROM stock WHERE symbol=? AND user_id=?", stock["symbol"], session["user_id"])
        if not preshare:
            return apology("Sorry you dont own that stock", 404)
        elif num > preshare[0]['shares']:
            return apology('Sorry you dont own that many stock', 403)
        elif num == preshare[0]['shares']:
            db.execute("DELETE FROM stock WHERE symbol=? AND user_id=?", stock["symbol"], session["user_id"])
        elif num < preshare[0]['shares']:
            db.execute("UPDATE stock SET shares=? WHERE symbol=? AND user_id=?", preshare[0]['shares']-num ,stock["symbol"], session["user_id"])

        availmon += num * stock_cost
        db.execute("UPDATE users SET cash = ? WHERE id = ?", availmon, session["user_id"])

        db.execute("INSERT INTO history(user_id, symbol, shares, price, transacted) VALUES(?, ?, ?, ?, ?)", session['user_id'], stock['symbol'], 0-num, stock_cost, dt_string )
        return redirect("/")
    else:
